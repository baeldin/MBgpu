...
void main() {
    ...
    {
        float xOffset = 0.5 * tent(wrap1(float(ii) / float(maxPasses), tofloat(lowbias32(uint(2 * pixelIdx    )))));
        float yOffset = 0.5 * tent(wrap1(halton(ii, 2),                tofloat(lowbias32(uint(2 * pixelIdx + 1)))));
